...
class KeyManager:
    ...
    def save_keys(
        ...
        self.console.print(
            f"[green]✓[/green] Keys saved to standard location:"
        )
    ...
    def setup_key_auth(
        ...
        try:
            ...
            self.console.print("[green]✓[/green] Generated ED25519 key pair")
            ...
            self.console.print("[green]✓[/green] Uploaded public key to server")

            # 4. Update ~/.ssh/config
            ssh_config_alias = self._update_ssh_config(server_config, private_path)
            self.console.print(f"[green]✓[/green] Updated SSH config (alias: {ssh_config_alias})")

            # 5. Test key login
            self.console.print("[yellow]→[/yellow] Testing SSH key login...")
            if self._test_key_login(server_config):
                self.console.print("[green]✓[/green] SSH key login successful!")
                ...
                self.console.print(
                    "[red]✗[/red] SSH key login failed. Rolling back..."
                )
            ...
            self.console.print(
                f"[red]✗[/red] Failed to setup SSH key: {e}"
            )
            return False
...
"""
    ...
    def remove_key_auth(self, server_config: ServerConfig, password: str) -> bool:
        ...
        try:
            ...
            self.console.print("[green]✓[/green] SSH key removed from server")

            return True

        except Exception as e:
            self.console.print(f"[red]✗[/red] Failed to remove SSH key: {e}")
            return False

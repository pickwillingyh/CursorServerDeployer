    ...
    def _find_cursor_executable(self) -> Optional[Path]:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info('OK Found cursor in PATH')
        ...
        for path in common_paths:
            if path.exists():
                logger.info(f'OK Found cursor at: {path}')
                return path
            else:
                logger.debug(f'ERROR Cursor not found at: {path}')
        ...
        if sys.platform == 'win32':
            ...
            for dir_path in path_dirs:
                ...
                if cursor_path.exists():
                    logger.info(f'OK Found cursor in PATH: {cursor_path}')
                    return cursor_path
                else:
                    logger.debug(f'ERROR Cursor not found in PATH directory: {dir_path}')

        logger.error('ERROR Cursor executable not found in any common locations')
        return None
